...
describe('Space Unit Modal', () => {
  ...
  it('Should validate and add Space Unit', () => {
    ...
    for(let i = 0; i < 3; i++){ 
      ...
      // cy.get('[data-cy="space-unit-input-error"]').should('be.visible')
      ...
      if(spaceUnit === 'space'){ // add showcase url
        ...
        cy.get('[data-cy="space-unit-showcase-url-error"]').should('be.visible')
      ...
      cy.get('[data-cy="email-input-wrapper"]').find('p').should('be.visible')
      // ------------------------------ 
...
const expectInit = (spaceUnit: string) => {
  ...
  cy.get('[data-cy="ctr-name-label"]').should('contain.text', '0/25')
  cy.get('[data-cy="ctr-name-label"]').should('not.have.class', 'has-text-danger')
...
const expectDispose = (payload: { spaceUnitName: string; }) => {
  ...
  cy.get('[data-cy="space-table-wrapper"]').find(`[data-cy="tbl-${payload.spaceUnitName}"]`).eq(0).should('have.text', payload.spaceUnitName)
